...
Return the final markdown list.`;
  ...
  private buildRelevantMemoryContext(
  ...
  private validateTaskMarkdown(markdown: string, contextFiles: Record<string, string>, originalMarkdown?: string): string {
    ...
    if (checkboxLines.length === 0 || !hasPhaseHeader) {
      ...
      if (originalMarkdown && originalMarkdown !== markdown) {
        console.warn(chalk.yellow('   [WARN] Rule-validated tasks lost formatting. Trying original tasks...'));
  ...
  private normalizeTaskLines(lines: string[], contextFiles: Record<string, string>): string[] {
    const availableFiles = Object.keys(contextFiles);
  ...
  private extractReferenceFiles(taskBody: string, availableFiles: string[]): string[] {
  ...
  private scoreTaskAgainstFile(taskDescription: string, fileName: string, content: string): number {
    let score = 0;
  ...
  private extractRelevantMemory(memoryMarkdown: string, taskDescription: string, referencedFiles: string[]): string {
  ...
  private scoreMemoryEntry(entry: string, taskTokens: Set<string>): number {
    let score = 0;
  ...
  private getPreferredContextOrder(availableFiles: string[]): string[] {
