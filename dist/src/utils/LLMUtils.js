...
function getLLMProvider(requestedProvider) {
    ...
    if (providerName) {
        switch (providerName) {
            ...
            case 'pro':
                ...
                if (process.env.GEMINI_API_KEY) {
                    ...
                    console.log(chalk_1.default.red('Error: "pro" requires GEMINI_API_KEY or OPENAI_API_KEY in .env.'));
                    console.log(chalk_1.default.yellow('Falling back to "free" (pollinations)...'));
                    providerName = 'pollinations';
                    config = { model: process.env.FREE_LLM_MODEL || 'openai' };
                }
                break;
            default:
                console.log(chalk_1.default.yellow(`Unknown provider "${providerName}". Falling back to automatic selection.`));
        ...
        if (providerName && providerName !== 'pollinations' && providerName !== 'local' && !config.apiKey) {
            console.log(chalk_1.default.red(`Error: API key for "${providerName}" is missing in .env.`));
            console.log(chalk_1.default.yellow(`Falling back to automatic selection...`));
    ...
    if (!providerName) {
        if (process.env.GEMINI_API_KEY) {
            ...
            providerName = 'pollinations';
            config = { model: process.env.FREE_LLM_MODEL || 'openai' };
            console.log(chalk_1.default.blue('Using FREE "Pollinations" LLM (No Key Required).'));
        }
    }
    console.log(chalk_1.default.gray(`LLM Provider: ${chalk_1.default.white(providerName)}`));
    return LLMFactory_1.LLMFactory.create(providerName, config);
}
